...
class PreferencesDialog(QDialog):
    # Signal to notify profile change: send (profile:str, path:Path)
    ...
    def _new_profile(self) -> None:
        ...
        # Create folder and seed minimum files if you want (optional)
        root = profile_root(name)
        root.mkdir(parents=True, exist_ok=True)
        (root / "backups").mkdir(parents=True, exist_ok=True)

        # Copy seeds if you are interested (empty food/allergens, for example)
        # Here we leave it empty for each profile to start clean.

        # Select new profile
        set_selected_profile(name)
        self._reload_profiles_ui()
        # Emit signal now (on-the-go change)
        self.profile_changed.emit(name, root)

    def _accept(self) -> None:
        # If the selected one was changed, apply and emit
    ...
    def _browse(self) -> None:
        ...
        if new:
            ...
            # Note: physically changing the global path would imply migrating profiles;
            # we leave it as an informational view.
